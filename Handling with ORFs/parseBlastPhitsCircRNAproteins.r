...
thr2=1
workDir="D:/Vasily Grinev"
res <- parseBlastPhitsCircRNAproteins(msProts=msProts,
                                      blastpHits=blastpHits,
                                      thr1=thr1,
                                      msPepts=msPepts,
                                      thr2=thr2,
                                      workDir=workDir)
suppressMessages(expr=library(package=Biostrings))
writeXStringSet(x=res[[2]],
                filepath="AML proteome, Kasumi-1, siMM for 3 days, #1, circRNA proteins, new peptides, first step, sequences.fasta")

x="6GUYPA5G013-Alignment.txt"
pr="fr4.fasta"
thr1=90
workDir="D:/Vasily Grinev"
suppressMessages(expr=library(package=Biostrings))
...
hits <- read.table(file=paste(workDir, x, sep = "/"),
                   sep="\t",
                   header=FALSE,
                   quote="\"",
                   as.is=TRUE)
...
writeXStringSet(subseqs, "fr5.fasta")
