...
## expr - path to folder and name of the txt file in tab-delimited format with experimentally detected
#  exon-exon junctions. This is input data file which should include five mandatory fields:
#  i) Event_ID (ID of the splicing event);
#  ii) Start (start genomic coordinate of the splicing event);
#  iii) End (end genomic coordinate of the splicing event);
#  iv) logFC (log2 fold change in usage of the splicing event between two conditions);
#  v) q_value (FDR-adjusted p-value for logFC).
...
#   source("D:/Vasily Grinev/overlapJunctions.r")
#   modelData = "D:/Vasily Grinev/Ensembl_GRCh38.p7_release.85.sqlite"
#   exprData = "D:/Vasily Grinev/PHF5A dataset, limma.diffSplice, exon-exon junctions.txt"
#   ctrl = c("siAGF1_1", "siAGF1_2", "siAGF1_3", "siAGF1_4", "siAGF1_5")
#   expr = c("siPHF5A_1", "siPHF5A_2", "siPHF5A_3", "siPHF5A_4")
#   res = overlapJunctions(models = modelData, experimental = exprData, ctrl.samples = ctrl, expr.samples = expr)
#   write.table(res$Expr.overlaps.experimental, file = "D:/Vasily Grinev/res.txt", sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
